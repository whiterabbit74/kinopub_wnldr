...
// URL для минимальных FFmpeg сборок
const FFMPEG_SOURCES = {
  // Используем статические сборки от John Van Sickle - меньше размер
  'darwin-x64': 'https://evermeet.cx/ffmpeg/ffmpeg-6.0.zip',
  'darwin-arm64': 'https://evermeet.cx/ffmpeg/ffmpeg-6.0.zip',
  // Для Windows используем essentials сборки вместо full
  'win32-x64': 'https://www.gyan.dev/ffmpeg/builds/packages/ffmpeg-6.0-essentials_build.zip',
  'win32-ia32': 'https://www.gyan.dev/ffmpeg/builds/packages/ffmpeg-6.0-essentials_build.zip'
};
...
async function downloadFFmpegForPlatform(platform) {
  ...
  try {
    ...
    const ffmpegBinary = await findBinary(tempDir, 'ffmpeg');
...
async function main() {
  console.log('📥 Downloading minimal FFmpeg binaries...\n');
